...
df <- read.csv('/home/Data/augmentation-effective/analyses/heatmap_esp.csv')
...
graphic <- ggplot(data = df, aes(x=method, y=dataset, fill=mean_p)) + 
  ...
  scale_fill_gradient2(limits=c(-20, 70), 
...
graphic
ggsave('/home/Data/augmentation-effective/analyses/specificity.pdf', 
       plot=graphic, 
       width = 12, 
       height = 10, 
       dpi = 300)
