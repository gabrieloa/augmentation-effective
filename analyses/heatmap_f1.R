...
df <- read.csv('/home/Data/augmentation-effective/analyses/heatmap_f1.csv')
...
graphic <- ggplot(data = df, aes(x=method, y=dataset, fill=mean_p)) + 
  ...
  scale_fill_gradient2(limits=c(-71, 550), 
...
graphic
ggsave('/home/Data/augmentation-effective/analyses/f1_score.pdf', 
       plot=graphic, 
       width = 15, 
       height = 10, 
       dpi = 300)
