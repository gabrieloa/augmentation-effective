...
df <- read.csv('/home/Data/augmentation-effective/analyses_full/heatmap_ba.csv')
...
graphic <- ggplot(data = df, aes(x=method, y=dataset, fill=mean_p)) + 
  ...
  facet_grid(~ factor(facet_a, levels = facet_levels))+
...
graphic
ggsave('/home/Data/augmentation-effective/analyses_full/heatmap_full.pdf', 
       plot=graphic, 
       width = 15, 
       height = 10, 
       dpi = 300)
